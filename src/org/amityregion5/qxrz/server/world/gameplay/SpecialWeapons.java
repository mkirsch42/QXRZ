...
public final class SpecialWeapons
{
	...
	public static void shotgun(World w, Player p, Vector2D v)
	{
		...
		Bullet b = new Bullet(entity.getPos(),
				new Vector2D(v.angle()).multiply(v.length()), p.getEquipped(),
				entity);
		if (team != null)
		...
		b = new Bullet(entity.getPos(),
				new Vector2D(v.angle() + 0.1).multiply(v.length()),
				p.getEquipped(), entity);
		if (team != null)
			b.setFriendlyFireTeam(team);
		b.setFriendlyFirePlayer(p);
		b.setSource(entity);
		...
		b = new Bullet(entity.getPos(),
				new Vector2D(v.angle() + 0.15).multiply(v.length()),
				p.getEquipped(), entity);
		if (team != null)
			b.setFriendlyFireTeam(team);
		b.setFriendlyFirePlayer(p);
