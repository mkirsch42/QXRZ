...
public final class Main
{
	...
	public Main(String[] args) throws Exception
	{
		...
		{
			s = (String) JOptionPane.showInputDialog(null, "Enter Server name",
					"Server Name Query", JOptionPane.PLAIN_MESSAGE, null, null,
		...
		Worlds world = (Worlds) JOptionPane.showInputDialog(null,
				"Choose World", "Choose World", JOptionPane.QUESTION_MESSAGE,
				null, Worlds.values(), Worlds.DEBUG);
		...
		g = new Game(netManager, GameModes.LASTMAN, this, world); // TODO game
																	// needs
																	// access to
																	// network,
																	// too...
	...
	private void doLobbyUpdate()
	{
		for (NetworkNode n : g.getPlayers().keySet())
		{
			...
			try
			{
				n.send(new LobbyInformationPacket(p.getSpecMove().getType(), p
						.isReady()));
			} catch (Exception e)
			{
				e.printStackTrace();
		...
		netManager.attachEventListener(new NetEventListener()
		{
			...
			public void dataReceived(NetworkNode c, Serializable netObj)
			{
				if (gs == GameState.LOBBY)
				{
					...
					{
						g.findPlayer(c).setReady(
								((ReadyPacket) netObj).isReady());
					...
					} else if (netObj instanceof ChangeClassPacket)
					{
						...
						p.setSpecMove(new SpecialMovement(SpecialMovements
								.values()[ordinal], g.getWorld()));
					}
				}
