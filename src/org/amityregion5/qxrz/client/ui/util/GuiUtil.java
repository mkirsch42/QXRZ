...
{
	public static void drawString(Graphics2D g, String str, CenterMode mode, int x, int y) {
	...
	public static Font scaleFont(String text, Rectangle rect, Graphics2D g) {
		...
		newSize *= rect.getWidth() / GuiMath.getStringBounds(g, text, 0, 0).getWidth();

		tempFont = g.getFont().deriveFont(newSize);
		g.setFont(tempFont);

		if (GuiMath.getStringBounds(g, text, 0, 0).getHeight() > rect.getHeight()) {
			newSize *= rect.getHeight() / GuiMath.getStringBounds(g, text, 0, 0).getHeight();
			tempFont = g.getFont().deriveFont(newSize);
			g.setFont(tempFont);
		}
