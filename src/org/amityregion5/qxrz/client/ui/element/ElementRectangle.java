...
public class ElementRectangle extends AGuiElement
{
	 ...
	 *            the function to determine to top left corner of the rectangle
	 * @param widthHeightFunction
	 *            the function to determine the width and height of the
	 *            rectangle
	 * @param background
	 ...
	 *            the border color
	 * @param sizeOrPadding
	 *            the size of the text or if it is a negative number to padding
	 *            for the resize
	 * @param text
	 *            the text color
	 * @param name
	...
	public ElementRectangle(Function<WindowData, Point> topLeftFunction,
			...
			Color border, float sizeOrPadding, Color text, String name)
	{
		this(topLeftFunction, widthHeightFunction, () -> background,
				() -> border, sizeOrPadding, () -> text, () -> name, null);
	}
	 ...
	 *            the text in the rectangle
	 * @param onClick
	 *            the click listener for the rectangle
	 */
	public ElementRectangle(Function<WindowData, Point> topLeftFunction,
			Function<WindowData, Point> widthHeightFunction, Color background,
			Color border, float sizeOrPadding, Color text, String name,
	...
	{
		this(topLeftFunction, widthHeightFunction, () -> background,
				() -> border, sizeOrPadding, () -> text, () -> name, onClick);
	}
	 ...
	 *            the text color
	 * @param name
	 *            the string supplier for the text in the rectangle
	 */
	public ElementRectangle(Function<WindowData, Point> topLeftFunction,
			Function<WindowData, Point> widthHeightFunction,
			Supplier<Color> background, Supplier<Color> border,
			float sizeOrPadding, Supplier<Color> text, Supplier<String> name,
			Consumer<WindowData> onClick)
	{
		this.name = name;
	...
	protected void draw(Graphics2D g, WindowData windowData)
	{
		...
		g.setColor(text.get());
		// Draw the text
		GuiUtil.drawString(g, name.get(), CenterMode.CENTER, getX()
	...
	/**
	 * @param widthHeightFunction
	 *            the widthHeightFunction to set
	 */
