...
public class MathUtil
{
	 ...
	 */
	public static double getAngleBetweenPoints(Point2D.Double start,
			Point2D.Double end)
	{
	 ...
	 * @param rad
	 *            the radian measure of the line from standard position
	 * @return the point at the end of this line
	 */
	public static Point2D.Double getPointAtEndOfLine(Point2D.Double start,
			double len, double rad)
	{
		return new Point2D.Double(start.getX() + len * Math.cos(rad),
				start.getY() + len * Math.sin(rad));
	}
